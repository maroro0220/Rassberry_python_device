...
class DISPLAY():
	def __init__(self):
		...
		self.frame2.pack(ipadx = 5, ipady = 5, fill=BOTH)
		self.frame3.pack(ipadx = 2, ipady = 2, fill=BOTH)
		self.frame4.pack(ipadx = 5, ipady = 5, fill=BOTH)
		self.frame5.pack(ipadx = 20, ipady = 20, fill=BOTH)
		self.frame6.pack(ipadx = 20, ipady = 5, fill=BOTH)
		self.frame7.pack(ipadx = 20, ipady = 50, fill=BOTH)
	...
	def ult(self):
		self.ultra_action.config(text="Distance chk\n"+str(self.ul.ultson())+" cm")
	...
	def PIR(self):
		...
		self.pi.loop()
		print(self.pi.loop())
		#self.clcd.lcd_string(self.pi.pir(),2)
		
	def cl(self):
		self.clcd.lcd_string(str(self.ul.ultson())+" cm",2)


	def event(self):
        ##1 LED
		self.led1_state = Label(self.frame1, text = "LED1 밝기 상태", font =20)
		self.led2_state = Label(self.frame1, text = "LED2 밝기 상태", font =20)
		...
		self.btn_led1_power.grid(row=1, column=2, padx=5, pady=5)
		self.btn_led2_power.grid(row=1, column=3, padx=5, pady=5)
		self.btn_led1_up.grid(row=2, column=2,padx=5, pady=5)
		self.btn_led1_dn.grid(row=2, column=3,padx=5, pady=5)
		self.btn_led2_up.grid(row=3, column=2,padx=5, pady=5)
		self.btn_led2_dn.grid(row=3, column=3,padx=5, pady=5)
		self.led1_state.grid(row = 2, column=4)
		self.led2_state.grid(row = 3, column=4)

		##2 모터
		self.btn_state = Label(self.frame2, text = "Btn  상태 ", font = 20)
		self.btn_motor_power = Button(self.frame2, width = 10, height = 2, text = "On", command = self.dcm)
		self.btn_motor_up = Button(self.frame2, width = 10, height = 2, text = "UP")
		self.btn_motor_dn = Button(self.frame2, width = 10, height = 2, text = "DN")
		self.btn_motor_dir = Button(self.frame2, width = 10, height = 2, text = "Spin 방향변경")

		self.btn_state.grid(row=1, column=0, padx=5, pady=5 )
		self.btn_motor_power.grid(row=2, column=1, padx=5, pady=5)
		self.btn_motor_dir.grid(row=2, column=2, padx=5, pady=5)
		...
		self.jog_monitor = Label(self.frame3, text="JogSwitch",width = 30, height = 3)
		self.jog_monitor.grid(row=1, column=3, padx=5, pady=5, columnspan=2)

                #4 clcd

		self.clcd_textbox =  Entry(self.frame4, text="값 입력창 ")
		self.clcd_textbox.grid(column=2, row=1, padx=5, pady=5)

		self.btn_clcd_ok = Button(self.frame4, text = "OK", width = 8, height = 2)
		self.btn_clcd_ok.grid(row=3, column=2, padx=5, pady=5)

		self.clcd_label = Label(self.frame4, text='문자를 입력하세요', height = 3)
		self.clcd_label.grid(column=1, row=1,padx=5, pady=5)
                #5 ultrasonic
		self.ultra_action = Button(self.frame5, text="거리값 알아보기", height = 2, command=self.ult)
		self.ultra_action.grid(column=2, row=1, padx=5, pady=5, columnspan=2)

		#6 piezo
		self.btn_piezo_power = Button(self.frame6, width = 12, height = 2, text = "On", command=self.piez)
		self.btn_piezo_power.grid(row=1, column=3, padx=5, pady=5, columnspan=2 )

		#7 PIR
		self.pir_label = Label(self.frame7, text='PIR: 감지여부', font = 20)
		self.pir_label.grid(column=2, row=1, padx=5, pady=5,)

		#8 lightsensor
		self.pir_label = Label(self.frame8, text='Light Sensor : 밝기값', font = 20)
		self.pir_label.grid(column=2, row=1, padx=5, pady=5)

		#9 FND
		self.fnd_label = Label(self.frame9, text='숫자를 입력하세요')
		self.fnd_label.grid(column=1, row=1, padx=5, pady=5)

		self.fnd_textbox =  Entry(self.frame9, text="값 입력창 ")
		self.fnd_textbox.grid(column=2, row=1, padx=5, pady=5)

		self.fnd_action = Button(self.frame9, text="OK", width = 8, height = 2)
		self.fnd_action.grid(column=2, row=2, padx=5, pady=5)

		#10 Temp/Humid
		self.pir_label = Label(self.frame10, text='Temp/Humid : 온/습도값', font = 20)
		self.pir_label.grid(column=2, row=1, padx=5, pady=5)
